...
const Hash = use('Hash')
const Tweet = use('App/Models/User')
...
class UserController {
  ...
  async me ({ auth, response }) {
    // https://scotch.io/courses/build-a-twitter-clone-with-adonis-and-vue/update-user-profile
  ...
  async usersToFollow ({ params, auth, response }) {
    ...
    // fetch users the currently authenticated user is not following
  ...
  async timeline ({ auth, response }) {
    const user = await User.find(auth.current.user.id) // TODO this may break. See line 212 for why

    // get an array of IDs of the users followed by current user
